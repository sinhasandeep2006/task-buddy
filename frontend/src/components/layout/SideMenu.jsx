    ...
    <div className="w-64 h-[calc(100vh-61px)] bg-white border-r border-gray-200/50 sticky top-[61px] z-20">
      <div className="flex flex-col items-center justify-center mb-7 pt-5">
        <div className="relative">
          <img src={user?.profileImageUrl || " "} alt="p i"  className="w-20 h-20 bg-slate-400 rounded-full"/>
        </div>
        ...
        {sideMenuData.map((item,index) => (
            <button
                key={`menu_${index}`}
                className={`flex items-center gap-4  rounded-md text-[15px] text-left hover:bg-gray-100 transition-colors duration-200 ${
                activeMenu === item.label ? "text-primary bg-linear-to-r from-blue-50/40 to-blue-100/50 border-r-3" : ""
                } py-2 px-3 cursor-pointer w-full `}
